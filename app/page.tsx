...
export default async function Home() {
  ...
  return (
    <>
      ...
      {/* 0. MOBILE QUICK SHOP — visible uniquement mobile, achat rapide */}
      <section className="md:hidden">
        <div className="px-4 pt-4 pb-2">
          ...
          <p className="mb-4 text-center text-xs text-[#6B7280]">
            Chariots électriques de golf premium — 2 accessoires offerts
          </p>
          ...
          <div className="grid grid-cols-2 gap-3">
            {trolleyMeta.map((trolley) => (
              <div
                key={trolley.slug}
                className="overflow-hidden rounded-xl border border-[#DBDBDB] bg-white"
              >
                {/* Image */}
                <div className="relative aspect-square bg-white p-2">
                  ...
                  <div className={`absolute top-2 left-2 rounded-full px-2 py-0.5 text-[10px] font-semibold ${trolley.badgeColor}`}>
                    {trolley.badge}
                  </div>
                ...
                {/* Info + CTA */}
                <div className="p-3">
                  <h3 className="text-sm font-bold text-[#0F0F10]">{trolley.name}</h3>
                  <div className="mt-1 flex items-baseline gap-1.5">
                    <span className="text-lg font-bold text-[#0F0F10]">{trolley.price}€</span>
                    {trolley.comparePrice && (
                      <span className="text-xs text-[#6B7280] line-through">{trolley.comparePrice}€</span>
                    )}
                  </div>
                  <AddToCartRedirect
                    slug={trolley.slug}
                  ...
                  <Link
                    href={trolley.href}
                    className="mt-1.5 block text-center text-[10px] font-medium text-[#356B0D] underline"
                  >
                    Voir le détail
                  </Link>
                </div>
      ...
      {/* 1. HERO — looping video background — desktop only */}
      <section className="relative hidden min-h-[560px] max-h-[960px] overflow-hidden sm:block sm:h-[90vh]">

...
- **Emails** : ZeptoMail (transactionnel, migration Resend en cours) + Zoho Mail Lite (boite commandes@powerbug.fr)
...
### Phase 1 — Structure + Pages produit (FAIT — 95%)
...
- [x] Hero homepage : vidéo lifestyle desktop + mobile (session 9)
- [x] Fiches produit trolleys (NX, NX DHC) avec galerie photos + specs
- [x] Fiches accessoires (/accessoires/[slug]) depuis Supabase
- [x] Page comparateur NX vs NX DHC (/trolleys) — redesign desktop session 15
...
- [x] Pages fonctionnalités (/fonctionnalites/[slug]) — 10 pages (8 initiales + batterie-lithium + VRAP)
- [x] Page contact (/contact) — formulaire + backend ZeptoMail
- [x] Auth/Login : page connexion/inscription + Google OAuth
- [x] 50 images officielles + 3 vidéos MP4 intégrées
- [x] 7 pages contenu : /notre-histoire, /garantie, /faq, /livraison, /cgv, /mentions-legales, /politique-retour
- [x] Page avis (/avis) — avis Trustpilot/Reviews.co.uk + tests presse

#### RESTE À FAIRE
- [ ] **Icône recherche header** : implémenter ou retirer

### Phase 2 — E-commerce (FAIT — 95%)
...
- [x] Bundle automatique : 2 accessoires OFFERTS avec chaque trolley (~60€)
- [x] Frais de livraison DPD : 14,90€ domicile / 3,90€ relais
- [x] Emails transactionnels ZeptoMail (confirmation client + bon préparation Golf des Marques)
- [x] Page compte client (commandes, adresses, factures)
- [x] Admin commandes (liste + détail + statut)
- [x] Stripe LIVE + Apple Pay + Google Pay + codes promo
- [x] Factures PDF (jsPDF, Supabase Storage, jointes aux emails)
- [x] DPD Relais Pickup (sélecteur point relais, SOAP MyPudo)
...
#### FAIT
- [x] Configurer messagerie @powerbug.fr (Zoho Mail Lite, commandes@powerbug.fr)
- [x] Basculer Stripe en production (clés LIVE Fred dans Vercel)
- [x] Test commande réelle (0.72€, webhook OK)
- [x] Domaine powerbug.fr → Vercel (redirection active)
- [x] Logo HD PowerBug (extrait du brand kit UK)
- [x] Compte Stripe Fred (créé le 14 mars 2026)
- [x] Script monitoring check-site.sh (37/38 tests OK)

#### En attente tiers
- [ ] Records DNS Resend : Jimmy doit ajouter DKIM + MX send + TXT send + DMARC (envoyé 20/03)
- [ ] Credentials DPD : en attente Golf des Marques
- [ ] Supabase → Pro : 1 clic upgrade (prêt)
